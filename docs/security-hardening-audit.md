...
- Admin API routes under `app/api/admin/**` call `requireAdminApi()` and now immediately return the guard response when authentication or authorization fails, preventing direct non-admin API calls from continuing into business logic.
- The manual paid-order route at `app/api/orders/[id]/mark-paid/route.ts` is an admin-only API even though it is outside the `/api/admin` path; it now also immediately returns the admin guard response.
