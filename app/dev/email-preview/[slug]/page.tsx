...
export default async function EmailPreviewPage({
  ...
  const emailHtml = await render(preview.render());

  return (
    <main className="min-h-screen bg-[#f7efe7] px-4 py-8">
      <div className="mx-auto mb-6 max-w-4xl rounded-2xl border border-[#ead8c4] bg-white p-5 shadow-sm">
        <Link className="text-sm font-bold text-[#9f6b3d]" href="/dev/email-preview">
          Back to all email previews
        </Link>
        ...
        <p className="mt-4 rounded-xl bg-[#f7efe7] px-4 py-3 text-sm font-semibold text-[#6b5a50]">
          Mock preview only. No email is sent from this page.
        </p>
      </div>
      <div className="mx-auto max-w-4xl overflow-hidden rounded-2xl border border-[#ead8c4] bg-white shadow-sm">
        <iframe
